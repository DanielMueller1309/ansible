...
def main():
    ...
    module = AnsibleModule(
        ...
        #required_if=[
        #                ('state', 'modify', ('database', 'title'), True),
        #                ('state', 'create', ("database"), True)
        #],
    )
    ...
    if state == 'create':
        try:
            if keyfile and not db_password:
                kp = pykeepass.create_database(database, keyfile=keyfile)
            if db_password and not keyfile:
                kp = pykeepass.create_database(database, password=db_password)
            if db_password and keyfile:
                kp = pykeepass.create_database(database, password=db_password, keyfile=keyfile)
        except:
            KEEPASS_OPEN_ERR = traceback.format_exc()
            module.fail_json(msg="Could not create Database File. Please verify that the path is set correctly and either set 'db_password' or the path to the 'keyfile'")
